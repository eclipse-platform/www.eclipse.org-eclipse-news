...
public class StatusHandlingView extends ViewPart {
	...
	private static final String XML_EXTENSIONS = "*.xml"; //$NON-NLS-1$
	...
	private final class TestBedSequenceJob extends Job {
		...
		private TestBedSequenceJob() {
			super(Messages.StatusHandlingView_TestBedSequence);
		}

		protected IStatus run(IProgressMonitor monitor) {
			monitor.beginTask(
					Messages.StatusHandlingView_ExecutingTestBedSequence,
					executionList.size());
	...
	public void createPartControl(Composite parent) {
		...
		addButton.setText(Messages.StatusHandlingView_AddToTheExecutionList);
		...
		executeAll.setText(Messages.StatusHandlingView_ExecuteAll);
		...
		removeButton.setText(Messages.StatusHandlingView_Remove);
		...
		exportButton.setText(Messages.StatusHandlingView_Export);
		...
		importButton.setText(Messages.StatusHandlingView_Import);
	...
	private void createExecuteButton(Composite parent) {
		...
		executeButton.setText(Messages.StatusHandlingView_ExecuteTab);
		executeButton
				.setToolTipText(Messages.StatusHandlingView_ExecuteTabTooltip);
	...
	private void storeState() {
		try {
			...
			fd.setText(Messages.StatusHandlingView_SaveDialogTitle);
			fd.setFilterExtensions(new String[] { XML_EXTENSIONS });
			String path = fd.open();
			if (path == null)
				return;
			BufferedWriter bw = new BufferedWriter(new FileWriter(
					new File(path)));
			XMLMemento memento = XMLMemento.createWriteRoot("runnables"); //$NON-NLS-1$
			for (int i = 0; i < executionList.size(); i++) {
				TestBedRunnable runnable = (TestBedRunnable) executionList
						.get(i);
	...
	private void restoreState() {
		try {
			...
			fd.setText(Messages.StatusHandlingView_LoadDialogTitle);
			fd.setFilterExtensions(new String[] { XML_EXTENSIONS });
			String path = fd.open();
			if (path == null)
				return;
